...
#
# - Bloombox - bloombox-partner-info
#
...
all: build
	@echo "bloombox-partner-info is ready."
...
ifeq ($(BUILDBOT),yes)
test:
	wct --job-name "bloombox-partner-info" --expanded --build-number "$(BUILD_NUMBER)" --sauce-tunnel-id "$(SAUCE_TUNNEL)" --plugin wct-jenkins
else
test:
	@wct --job-name "bloombox-partner-info"
endif
